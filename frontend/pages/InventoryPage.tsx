...
const InventoryPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchAreas = async () => {
            try {
                ...
                console.error("Failed to fetch areas for filter", error);
            }
        };
        fetchAreas();
    }, []);
    ...
    return (
        <div className="container mx-auto p-6">
            ...
            <div className="border-b border-slate-200 dark:border-slate-700 mb-6">
                <nav className="-mb-px flex space-x-6" aria-label="Tabs">
                    <button
                        ...
                        className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'equipment' ? activeTabClasses : inactiveTabClasses}`}
                    >
                        Equipment
                    </button>
                    <button
            ...
            <div>
                {activeTab === 'equipment' && <EquipmentManagement searchQuery={searchQuery} areaFilter={areaFilter} />}
                {activeTab === 'rooms' && <RoomManagement searchQuery={searchQuery} areaFilter={areaFilter} />}
            </div>
        </div>
    );
};
