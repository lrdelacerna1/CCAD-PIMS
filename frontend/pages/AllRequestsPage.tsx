...
const StatusBadge: React.FC<{ status: AnyRequest['status'] }> = ({ status }) => {
    ...
    const colors: { [key: string]: string } = {
        ...
        'in': 'bg-cyan-100 text-cyan-800 dark:bg-cyan-900/40 dark:text-cyan-300',
    ...
    return <span className={`${baseClasses} ${colors[colorKey] || 'bg-slate-100 text-slate-800'}`}>{status}</span>;
};

const AllRequestsPage: React.FC = () => {
    const { user } = useAuth();
    const location = useLocation();
    const hasRunTabLogic = useRef(false);
    ...
    useEffect(() => {
        ...
        if (initialStatus && requests.length > 0 && !hasRunTabLogic.current) {
            ...
            if (hasPendingRoomRequests) setActiveTab('room');
            hasRunTabLogic.current = true;
        }
    }, [requests, location.state]);

    const handleStatusUpdate = async (
        request: AnyRequest,
        newStatus: EquipmentRequestStatus | RoomRequestStatus,
        action: string = 'update'
    ) => {
        setActionInProgress({ id: request.id, action });
        setError('');
        try {
    ...
    const managedAreaIds = useMemo(() => {
        ...
        return new Set(
            areas
                .filter(a => a.adminIds?.includes(user?.id || ''))
                .map(a => a.id)
        );
    }, [areas, user]);
    ...
    const filteredAndSortedRequests = useMemo(() => {
        ...
        let processed = requests.filter(r => {
        ...
        if (statusFilter !== 'all') {
            processed = processed.filter(r => {
                if (activeTab === 'room' && statusFilter === 'Ready for Pickup') {
                    return r.status === 'Ready for Pickup' || r.status === 'Ready for Check-in';
                }
        ...
        return processed;
    }, [requests, user, managedAreaIds, areaFilter, statusFilter, searchQuery, sortOrder, activeTab]);

    const areaFilterOptions = useMemo(() => {
        const base = [{ value: 'all', label: 'All Managed Areas' }];
        const relevant = areas.filter(a => managedAreaIds.has(a.id));
        return [...base, ...relevant.map(a => ({ value: a.id, label: a.name }))];
    }, [areas, managedAreaIds]);
    ...
    const renderActions = (request: AnyRequest) => {
        switch (request.status) {
            case 'Pending Approval':
                return (
                    <div className="flex gap-2">
                        <Button
                            onClick={(e) => { e.stopPropagation(); handleStatusUpdate(request, 'Approved', 'approve'); }}
                            isLoading={actionInProgress?.id === request.id && actionInProgress?.action === 'approve'}
                            variant="success"
                            className="!w-auto !px-2 !py-1 text-xs"
                        >Approve</Button>
                        <Button
                            onClick={(e) => { e.stopPropagation(); handleStatusUpdate(request, 'Rejected', 'reject'); }}
                            isLoading={actionInProgress?.id === request.id && actionInProgress?.action === 'reject'}
                            variant="danger"
                            className="!w-auto !px-2 !py-1 text-xs"
                        >Reject</Button>
                    </div>
            ...
            case 'Approved':
                ...
                return (
                    <Button
                        onClick={(e) => { e.stopPropagation(); handleStatusUpdate(request, readyStatus, 'ready'); }}
                        isLoading={actionInProgress?.id === request.id && actionInProgress?.action === 'ready'}
                        variant="primary"
                        className="!w-auto !px-2 !py-1 text-xs"
            ...
            case 'Ready for Pickup':
                return (
                    <Button
                        onClick={(e) => { e.stopPropagation(); handleStatusUpdate(request, 'In Use', 'checkout'); }}
                        isLoading={actionInProgress?.id === request.id && actionInProgress?.action === 'checkout'}
                        variant="dark"
                        className="!w-auto !px-2 !py-1 text-xs"
                    >Check Out</Button>
                );
            case 'Ready for Check-in':
                return (
                    <Button
                        onClick={(e) => { e.stopPropagation(); handleStatusUpdate(request, 'In Use', 'checkin'); }}
                        isLoading={actionInProgress?.id === request.id && actionInProgress?.action === 'checkin'}
                        variant="dark"
                        className="!w-auto !px-2 !py-1 text-xs"
                    >Check In</Button>
                );
            case 'In Use':
                ...
                return (
                    <Button
                        onClick={(e) => { e.stopPropagation(); handleStatusUpdate(request, closeStatus, 'close'); }}
                        isLoading={actionInProgress?.id === request.id && actionInProgress?.action === 'close'}
                        variant="dark"
                        className="!w-auto !px-2 !py-1 text-xs"
                    >Close</Button>
                );
                ...
                <>
                    <div className="flex justify-between items-center mb-6">
                        <p className="text-slate-500 dark:text-slate-400">
                            Showing {filteredAndSortedRequests.length} {activeTab} reservation(s).
                        </p>
                                    ...
                                    {filteredAndSortedRequests.length > 0 ? (
                                        filteredAndSortedRequests.map(req => (
                                            ...
                                            >
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center gap-2">
                                                        {(() => {
                                                            ...
                                                            if (!userForRequest) return <UserIcon className="w-5 h-5 text-slate-400 flex-shrink-0" title="User" />;
                                                            switch (userForRequest.role) {
                                                                case 'student': return <AcademicCapIcon className="w-5 h-5 text-slate-500 flex-shrink-0" title="Student" />;
                                                                case 'faculty': return <AcademicCapIcon className="w-5 h-5 text-purple-500 flex-shrink-0" title="Faculty" />;
                                                                case 'admin':
                                                                case 'superadmin': return <BriefcaseIcon className="w-5 h-5 text-slate-500 flex-shrink-0" title="Admin" />;
                                                                default: return <UserIcon className="w-5 h-5 text-slate-400 flex-shrink-0" title="User" />;
                                                            }
                                                        })()}
                                                ...
                                                <td className="px-6 py-4">
                                                    ...
                                                    {' at '}
                                                    {'requestedStartTime' in req ? (req as RoomRequest).requestedStartTime || 'N/A' : 'N/A'}
                                                </td>
                                                <td className="px-6 py-4">
                                        ...
                                        <tr>
                                            <td colSpan={5} className="text-center p-6">
                                                <p className="text-slate-500 dark:text-slate-400">
                                                    No {activeTab} reservations match your criteria.
                                                </p>
                                            </td>
                                        </tr>
                                    )}
