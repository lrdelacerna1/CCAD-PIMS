...
const RoomCatalog: React.FC<RoomCatalogProps> = ({ roomTypes, areas, isLoading, error, onSelectInstances, onViewDetailsClick, cartedInstanceIds }) => {
    ...
    return (
        <div>
            ...
            {isLoading ? (
                <p className="dark:text-white text-center">Loading rooms...</p>
            ) : error ? (
                    ...
                    {filteredRoomTypes.length > 0 ? filteredRoomTypes.map(rt => {
                        ...
                        return (
                            <Card key={rt.id} className="!p-0 !max-w-none flex flex-col transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
                                {rt.photoUrl
                                    ? <img src={rt.photoUrl} alt={rt.name} className="h-36 w-full object-cover rounded-t-xl" />
                                    ...
                                    <div>
                                        <div className="flex items-start justify-between mb-1 gap-1">
                                            <div className="flex-grow min-w-0">
                                                <p className="font-bold text-sm dark:text-white mb-0.5 truncate">{rt.name}</p>
                                                <p className="text-xs text-slate-500 dark:text-slate-400 truncate">{areasMap.get(rt.areaId) || 'Unknown Area'}</p>
                                            </div>
                                            <AvailabilityBadge status={rt.availabilityStatus} />
                                        </div>
                                        <div className="text-xs text-slate-600 dark:text-slate-300 mt-2">
                                            Available: <span className="font-semibold">{rt.availableForDates}</span>
                                        </div>
                                    </div>
